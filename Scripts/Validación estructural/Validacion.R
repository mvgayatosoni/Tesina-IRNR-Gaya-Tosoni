...
lines(final_$time, ini_$m, lty = 2)
lines(final_$time, final_$m, lty=1)
legend("topleft", c("datos", "ajuste calibración final", "ajuste validación"), lty = c(NA,2,1), pch = c(1, NA, NA), inset = c(-0.1, -0.499), xpd = TRUE)
plot(Data_n_real, xlab = "t (semanas)", ylab = "estériles",type = "b", pch = 16, col = "orange", lty = 2)
lines(final_$time, ini_$n, lty = 2)
lines(final_$time, final_$n,lty=1)
par(mfrow = c(1, 1))
